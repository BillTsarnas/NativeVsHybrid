...
var app = angular.module('MainActivity', ['ionic']);
	...
	app.service('timeService', function($http) {
		...
		var timer = function(taskType, time){
									var obj = {task : taskType, execution : time, platform : 'HYBRID', os_version : ionic.Platform.device().version};
									console.log(obj);
	...
	app.controller('leafletController', function($ionicPlatform, $location, $scope, $http, timeService) {
		...
		if($scope.leaf_map == ""){
			...
			//map.on "load" is fired when setView finishes. we want to measure the map init time 
  ...
  $scope.leaf_map="";
	...
	$scope.showJson = function(){
									$scope.result="Fetching data from server...";
	...
	$scope.showVisual = function(){
									//by the time the user starts clicks, we start counting
	...
	$scope.GETJson = function(){
									...
									.then(function(response){ $scope.result = "Data downloaded successfully!"; $scope.time = new Date().getTime() - start; timeService.timer("GET_JSON", $scope.time);}
									,function(response){ $scope.result = "Download Failed"; $scope.time = new Date().getTime() - start;});
	...
	$scope.POSTJson = function(){
									...
									var start = Date.now();
									$http.post("http://83.212.86.247/thesis/test.php", obj)
									.then(function(response){ $scope.POSTresult = "Data uploaded successfully!"; $scope.POSTtime = new Date().getTime() - start; timeService.timer("POST_JSON", $scope.POSTtime);}
									,function(response){ $scope.POSTresult = "Upload failed"; $scope.POSTtime = new Date().getTime() - start;});
